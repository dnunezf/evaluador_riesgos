...
<head>
    ...
    <style>
        :root{ --w: 900px; --h: 320px; --border:#e5e7eb; --fg:#0f172a; --ok:#16a34a; --crit:#ef4444; }
        *{box-sizing:border-box}
        body{ margin:0; padding:16px; font:500 16px/1.5 system-ui,Segoe UI,Roboto,Arial; color:var(--fg); background:#fff; }
        h2{ margin:0 0 12px 0; }
        .card{ width:var(--w); border:1px solid var(--border); border-radius:12px; padding:12px; box-shadow:0 6px 22px rgba(0,0,0,.06); background:#fff; }
        .hud{ display:inline-flex; gap:10px; align-items:center; border:1px solid var(--border); border-radius:999px; padding:6px 10px; font-weight:600; background:#fff; position:relative; z-index:2;}
        .hud .dot{ width:8px; height:8px; border-radius:50%; background:var(--ok); box-shadow:0 0 0 3px rgba(22,163,74,.15); }
        .hud.crit .dot{ background:var(--crit); box-shadow:0 0 0 3px rgba(239,68,68,.15); }
        .hud .sep{ color:#64748b; }
        .row{ display:flex; justify-content:space-between; align-items:center; margin-bottom:8px; }
        #estado{ font-weight:600 }
        #canvas{ width:var(--w); height:var(--h); display:block; } /* alto/ancho fijos: la pantalla no “salta” */
    </style>
</head>
<body>
<h2>Monitor</h2>
...
<script>
    ...
    function addPoint(tsISO, pct){
        ...
        if (y >= UMBRAL) { hud.classList.add('crit'); estadoEl.textContent = `CRÍTICO (≥ ${UMBRAL}%).`; estadoEl.style.color = '#ef4444'; }
        else { hud.classList.remove('crit'); estadoEl.textContent = `OK (< ${UMBRAL}%).`; estadoEl.style.color = '#16a34a'; }
    }
    ...
    async function tick(){
        try{
            ...
            estadoEl.textContent = 'Sin datos'; estadoEl.style.color = '#64748b';
            console.error(e);
        }
    }
    ...
    (function seed(){
        ...
        for (let i = 5; i >= 1; i--) {
            ...
            chart.data.datasets[1].data.push({ x: t, y: UMBRAL });
        }
        chart.update('none');
    })();
