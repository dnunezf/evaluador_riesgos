...
if (!isset($clients[$cliente]) || empty($clients[$cliente]['dblink'])) {
  ...
  echo json_encode(['ok' => 0, 'step' => 'init', 'error' => 'cliente_inválido_o_sin_dblink']);
  exit;
}
define('DBLINK_NAME', $clients[$cliente]['dblink']);
define('CLIENTE', $cliente);
define('UMBRAL', (int)($clients[$cliente]['umbral'] ?? 30));
...
if ($pct >= UMBRAL) {
  ...
  if ($clobSql) {
    oci_bind_by_name($ia, ':sqltxt', $clobSql, -1, OCI_B_CLOB);
  } else {
    oci_bind_by_name($ia, ':sqltxt', $sqltxt);
  }
  oci_bind_by_name($ia, ':consS', $consS);                // string
  if ($clobDet) {
    oci_bind_by_name($ia, ':det', $clobDet, -1, OCI_B_CLOB);
  } else {
    oci_bind_by_name($ia, ':det', $det);
  }
