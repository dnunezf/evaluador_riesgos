...
oci_execute($st1);
$dow = [];
while ($r = oci_fetch_assoc($st1)) { $dow[] = ['dow'=>(int)$r['DOW'], 'avg_pct'=>(float)$r['AVG_PCT']]; }
...
oci_execute($st2);
$hour = [];
while ($r = oci_fetch_assoc($st2)) { $hour[] = ['hh'=>(int)$r['HH'], 'avg_pct'=>(float)$r['AVG_PCT']]; }
...
oci_execute($st3);
$trend = [];
while ($r = oci_fetch_assoc($st3)) { $trend[] = ['day'=>$r['DIA'], 'max_pct'=>(float)$r['MAX_PCT']]; }
...
oci_execute($st4);
$alerts = [];
while ($r = oci_fetch_assoc($st4)) { $alerts[] = ['dia'=>$r['DIA'], 'hora'=>$r['HORA'], 'consumo'=>(float)$r['CONSUMO']]; }
